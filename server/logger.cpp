...
std::basic_ostream<Char, Traits>& operator<<(
    std::basic_ostream<Char, Traits>& os, Logger::Sequence& sequence) {
...
Logger::Logger(const std::string_view prefix, const std::string_view name)
    : m_file{openFile(prefix, name)} {
  m_file << m_sequence << "---=== Logger \"" << name << "\" stated ===---"
         << std::endl;
}

Logger::~Logger() {
  try {
    m_file << m_sequence << "^^^ ^^^ ^^^ Logger stopped ^^^ ^^^ ^^^"
           << std::endl;
  } catch (...) {
    assert(false);
  }
...
void Logger::putRecord(std::string_view record) {
